...
| ☐ | ESP32 + waterproof ultrasonic sensor (softener salt level) | 1 | ~$15 | "Add salt" alerts from the brine tank; salt % on the dashboard | Amazon: search "ESP32 dev board" + "JSN-SR04T" — docs/12 |
| ☐ | Zigbee energy-monitoring plug (softener head) | 1 | ~$15 | Regen-cycle detection → salt-bridge/stuck-valve alert after 10 quiet days | Amazon: search "Zigbee smart plug energy monitoring" — docs/12 |
| ☐ | Leak sensor for the softener drip pan (one more SNZB-05P) | 1 | ~$15 | Softener joins the leak/auto-shutoff coverage | same as leak-sensor link above |
