...
| ☐ | ESP32 + waterproof ultrasonic sensor + IP65 enclosure (softener salt level) | 1 | ~$20 | "Add salt" alerts from the brine tank; salt % on the dashboard | Amazon: search "ESP32 dev board" + "JSN-SR04T" — docs/12 |
| ☐ | Outdoor-rated Zigbee energy-monitoring plug (softener head) | 1 | ~$20 | Regen-cycle detection → salt-bridge/stuck-valve alert after 10 quiet days | Amazon: search "Zigbee smart plug energy monitoring" — docs/12 |
